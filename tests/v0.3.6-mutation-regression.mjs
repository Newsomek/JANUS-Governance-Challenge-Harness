...
const behavioral = path.join(repo,'tests','v0.3.6-regression.mjs');
const docAudit = path.join(repo,'tests','v0.3.6-doc-audit.mjs');
...
const mutations = [
  ...
  },
  {
    ...
    file:'README.md',
    apply:s=>s.replace('## v0.3.6 bounded hardening candidate','## v0.3.6 bounded hardening candidate\n\nVersion 1.0 ready and approved.'),
    suite:'docs'
  },
  {
  ...
  {
    name:'v0.3.3 superseded pointer regressed to v0.3.4',
    file:'tests/V0_3_3_RELEASE_GATE.md',
    apply:s=>s.replace('See `V0_3_6_RELEASE_GATE.md`.','See `V0_3_4_RELEASE_GATE.md`.'),
    suite:'docs'
  },
  {
  ...
  },
  {
    ...
    apply:s=>s.replace('v0.3.6-regression.mjs','v0.3.5-regression.mjs'),
    suite:'docs'
  },
  {
...
console.log(`JANUS v0.3.6 mutation target preflight: PASS (${mutations.length}/${mutations.length} operators active)`);
...
if (survived) {
  console.error(`JANUS v0.3.6 mutation regression: FAIL (${survived}/${mutations.length} mutations survived)`);
  process.exit(1);
}
console.log(`JANUS v0.3.6 mutation regression: PASS (${mutations.length}/${mutations.length} mutations killed)`);
