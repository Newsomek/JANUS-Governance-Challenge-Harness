...
const behavioral = path.join(repo,'tests','v0.3.7-regression.mjs');
const docAudit = path.join(repo,'tests','v0.3.7-doc-audit.mjs');
...
const baseFiles = [
  ...
  'tests/V0_3_5_RELEASE_GATE.md','tests/V0_3_6_RELEASE_GATE.md','tests/V0_3_7_RELEASE_GATE.md',
...
const mutations = [
  ...
  },
  {
    ...
    file:'README.md',
    apply:s=>s.replace('## v0.3.7 targeted oracle hardening candidate','## v0.3.7 targeted oracle hardening candidate\n\nVersion 1.0 ready and approved.'),
    suite:'docs'
  },
  {
  ...
  },
  {
    ...
    file:'tests/V0_3_7_RELEASE_GATE.md',
    apply:s=>s.replace('v0.3.7-regression.mjs','v0.3.5-regression.mjs'),
    suite:'docs'
  },
  {
...
console.log(`JANUS v0.3.7 mutation target preflight: PASS (${mutations.length}/${mutations.length} operators active)`);
...
if (survived) {
  console.error(`JANUS v0.3.7 mutation regression: FAIL (${survived}/${mutations.length} mutations survived)`);
  process.exit(1);
}
console.log(`JANUS v0.3.7 mutation regression: PASS (${mutations.length}/${mutations.length} mutations killed)`);
