...
const sourcePath = path.join(repo, 'docs', 'JANUS_Orientation_Edition_2026_EN.docx');
...
const build = {
  version: '0.3.3',
  build_id: 'janus-governance-challenge-harness-v0.3.3',
...
class MockElement {
  ...
  replaceChildren(...items) {
    this.children = [];
    if (items.length) this.children.push(...items);
  }
  remove() {}
...
const expose = `\n;globalThis.__janus={scenarios,labels,VALID_PREDICTIONS,VALID_SUPPORT_STATUSES,selectedScenario,validPrediction,validateScenarioContract,predictionComparison,evidenceCoreHash,recordSnapshotHash,runChallenge,replayLastRun,exportEvidence,resetHarness,deriveReplayRecord,observeHarnessProvenance,observeSourceDocument,get lastRun(){return lastRun},get lastReplay(){return lastReplay},set lastRun(v){lastRun=v},set lastReplay(v){lastReplay=v}};`;
...
console.log('JANUS v0.3.3 automated regression: PASS');
