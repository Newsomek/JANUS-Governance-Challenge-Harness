...
const build = {
  version: '0.3.14',
  build_id: 'janus-governance-challenge-harness-v0.3.14',
...
const indexText = fs.readFileSync(path.join(repo, 'index.html'), 'utf8');

assert(
  indexText.includes('<title>JANUS Governance Challenge Harness v0.3.14</title>'),
  'Public page title is not v0.3.14.'
);

assert(
  indexText.includes('EXTERNAL ARCHITECTURAL CONFORMANCE CHALLENGE · VERSION 0.3.14'),
  'Visible page eyebrow is not v0.3.14.'
);

assert(
  indexText.includes('<strong>JANUS Governance Challenge Harness v0.3.14</strong>'),
  'Public page footer is not v0.3.14.'
);
...
assert(
  !Object.prototype.hasOwnProperty.call(expectedHashes, 'version'),
  'Verifier-facing contract hash oracle must not expose an ambiguous active release version field.'
);
assert(
  expectedHashes.contracts_last_changed_in === '0.3.9',
  'Verifier-facing contract hash oracle must preserve explicit contract-origin metadata.'
);
...
// v0.3.9: invisible/default-ignorable text must be rejected.
...
// v0.3.9: duplicate commitments must be rejected.
{
  const sample = clone(j.scenarios[0]);
...
// v0.3.9: stored-record structural corruption must fail with stable, non-engine messages.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.9: unknown stored scenario_id must blame the stored record, not the authored contract.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.9: Run must remain locked out while Replay is in flight.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.9: Run must remain locked out while Export is in flight, and no export may be silently superseded.
...
// v0.3.9: canonical duplicate comparison must reject visually/semantically equivalent variants.
{
...
// v0.3.9: internal blank-rendering separators map to ordinary spaces; U+007F is not visible content.
{
...
// v0.3.9: explicit source SHA mismatch must refuse Run.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
{
...
// v0.3.9: bound-commit content mismatch must refuse rather than degrade to UNAVAILABLE.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
{
...
// v0.3.9: an uncoordinated stored-record-only edit must fail record snapshot integrity.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.9: selection change after a valid Run must invalidate evidence and preserve the notice.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.9: Replay completion after an intervening selection change must be discarded.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.9: Export must re-check generation after late hashing and before download.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.9 F04: Export event_log integrity must be behaviorally enforced.
...
console.log('JANUS v0.3.14 automated regression: PASS');
