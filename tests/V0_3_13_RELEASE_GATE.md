# JANUS Governance Challenge Harness v0.3.13 Release Gate

Version 0.3.13 is a bounded residual-hardening candidate.

V035-F04 remains independently CLOSED.

V035-F05 remains PARTIALLY CLOSED until independent v0.3.13 targeted testing returns zero actionable findings.
...
- the deterministic v0.3.13 build-info generator.
...
Both provenance files must equal the exact canonical JSON serialization accepted by the v0.3.13 validators. Duplicate keys, reordered keys, alternate whitespace, Unicode-escape alternatives, uppercase commit/tree/hash strings, extra keys and missing keys are refused.
...
1. `node tests/v0.3.13-regression.mjs`
2. `node tests/v0.3.13-residual-fixtures.mjs`, including `tests/v0.3.13-release-lifecycle-fixtures.mjs`
...
9. visible title / eyebrow / footer all identify v0.3.13
10. real v0.3.13 release-state and manifest audits fail closed while still unbound

## After Commit A

11. record exact Commit A SHA
12. record exact Commit A Git tree
13. run `tests/v0.3.13-bind-release-state.mjs` with explicit expected Commit A/tree
14. run `tests/v0.3.13-generate-build-info.mjs` with explicit expected Commit A/tree
...
23. run v0.3.13 behavioral regression
24. run v0.3.13 residual fixtures
...
Even after targeted closure, Version 1.0 remains blocked until a separate full independent zero-open-finding adversarial regression passes against the exact same deployed v0.3.13 artifact.
