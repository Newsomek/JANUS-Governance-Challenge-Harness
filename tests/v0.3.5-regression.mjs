...
const build = {
  version: '0.3.5',
  build_id: 'janus-governance-challenge-harness-v0.3.5',
...
// v0.3.5: invisible/default-ignorable text must be rejected.
...
// v0.3.5: duplicate commitments must be rejected.
{
  const sample = clone(j.scenarios[0]);
...
// v0.3.5: stored-record structural corruption must fail with stable, non-engine messages.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.5: unknown stored scenario_id must blame the stored record, not the authored contract.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.5: Run must remain locked out while Replay is in flight.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.5: Run must remain locked out while Export is in flight, and no export may be silently superseded.
...
// v0.3.5: canonical duplicate comparison must reject visually/semantically equivalent variants.
{
...
console.log('JANUS v0.3.5 automated regression: PASS');
