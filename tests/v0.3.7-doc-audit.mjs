...
const gate = read('tests/V0_3_7_RELEASE_GATE.md');
...
assert(readme.includes('# JANUS Governance Challenge Harness v0.3.7'), 'README current version missing.');
assert(version.includes('Version 0.3.7'), 'VERSION current version missing.');
assert(html.includes('JANUS Governance Challenge Harness v0.3.7'), 'UI version not v0.3.7.');
assert(hashes.version === '0.3.7', 'Expected contract hash file version mismatch.');
...
function withoutAllowedTamperNegations(text) {
  return text
    .replace(/\b(?:not|never)\b[^.\n]{0,100}\btamper[- ]proof\b/gi, '')
    .replace(/\bcannot\b[^.\n]{0,100}\btamper[- ]proof\b/gi, '');
}
...
for (const [name, text] of tamperSurfaces) {
  assert(
    !/tamper[- ]proof/i.test(withoutAllowedTamperNegations(text)),
...
const changelogCurrent =
  changelog.split('## v0.3.7')[1]?.split('\n## ')[0] || '';

claimSurfaces.push(['CHANGELOG v0.3.7', changelogCurrent]);

const version1Claims = [
  /Version 1\.0\s+(?:is\s+)?(?:ready|approved|validated|eligible|complete|certified)/i,
  /(?:ready|approved|validated|eligible|certified)\s+(?:for\s+)?Version 1\.0/i
];

const closureClaims = [
  /independent closure[^.\n]{0,50}(?:confirmed|complete|closed|approved|achieved|established)/i,
  /(?:confirmed|complete|approved|achieved|established)[^.\n]{0,50}independent closure/i,
  /V035-F01\s+through\s+V035-F09[^.\n]{0,50}independently closed/i
];

for (const [name, text] of claimSurfaces) {
  for (const rx of version1Claims) {
    assert(!rx.test(text), `${name} contains premature Version 1.0 claim: ${rx}`);
  }

  const closureText = text
    .replace(/Independent closure is \*\*not\*\* claimed[^.\n]*/gi, '')
    .replace(/Independent closure is not claimed[^.\n]*/gi, '')
    .replace(/Independent closure:\s*pending[^.\n]*/gi, '');

  for (const rx of closureClaims) {
    assert(!rx.test(closureText), `${name} contains premature independent-closure claim: ${rx}`);
  }
}
...
for (const suite of ['tests/v0.3.7-regression.mjs','tests/v0.3.7-doc-audit.mjs','tests/v0.3.7-mutation-regression.mjs','tests/v0.3.7-manifest-audit.mjs']) {
...
assert(readme.includes('V0_3_7_RELEASE_GATE.md'), 'README current gate missing.');
assert(gate.includes('targeted independent retest of residual V035-F04 and V035-F05'), 'Current gate missing targeted residual F04/F05 closure requirement.');
...
for (const [name,text] of [['v0.3.2',gate2],['v0.3.3',gate3],['v0.3.4',gate4],['v0.3.5',gate5],['v0.3.6',gate6]]) {
  assert(text.startsWith('# SUPERSEDED'), `${name} gate is not marked superseded.`);
}
for (const text of [gate2,gate3,gate4,gate5]) {
  assert(text.startsWith('# SUPERSEDED'), 'Historical gate is not marked superseded.');
}
assert(
  gate6.includes('V0_3_7_RELEASE_GATE.md'),
  'v0.3.6 superseded gate does not point to current v0.3.7 gate.'
);

assert(changelog.trimStart().startsWith('# Changelog'), 'CHANGELOG title is not first.');
assert(changelog.indexOf('## v0.3.7') > changelog.indexOf('# Changelog'), 'v0.3.7 entry missing or misplaced.');
...
const currentSection = readme.split('## v0.3.7 targeted oracle hardening candidate')[1] || '';
assert(currentSection.length > 0, 'README v0.3.7 section missing.');
...
console.log('JANUS v0.3.7 documentation/release audit: PASS');
