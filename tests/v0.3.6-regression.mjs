...
const build = {
  version: '0.3.6',
  build_id: 'janus-governance-challenge-harness-v0.3.6',
...
assert(expectedHashes.version === '0.3.6', 'Expected contract hash metadata version mismatch.');
...
// v0.3.6: invisible/default-ignorable text must be rejected.
...
// v0.3.6: duplicate commitments must be rejected.
{
  const sample = clone(j.scenarios[0]);
...
// v0.3.6: stored-record structural corruption must fail with stable, non-engine messages.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.6: unknown stored scenario_id must blame the stored record, not the authored contract.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.6: Run must remain locked out while Replay is in flight.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.6: Run must remain locked out while Export is in flight, and no export may be silently superseded.
...
// v0.3.6: canonical duplicate comparison must reject visually/semantically equivalent variants.
{
...
// v0.3.6: internal blank-rendering separators map to ordinary spaces; U+007F is not visible content.
{
...
// v0.3.6: explicit source SHA mismatch must refuse Run.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
{
...
// v0.3.6: bound-commit content mismatch must refuse rather than degrade to UNAVAILABLE.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
{
...
// v0.3.6: an uncoordinated stored-record-only edit must fail record snapshot integrity.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.6: Replay completion after an intervening selection change must be discarded.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.6: Export must re-check generation after late hashing and before download.
j.resetHarness();
elements.get('scenarioSelect').value = j.scenarios[0].id;
elements.get('predictionSelect').value = 'BLOCK_REAUTHORIZE';
await j.runChallenge();
...
// v0.3.6: preserve explicit event-log integrity check in Export.
assert(appText.includes('event_log: JSON.stringify(expectedLog) === JSON.stringify(snapshot.event_log)'), 'Export event-log integrity check is missing.');

console.log('JANUS v0.3.6 automated regression: PASS');
